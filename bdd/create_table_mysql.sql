...
CREATE TABLE T_UNIT_UNT (
  ...
  FOREIGN KEY (USR_ID) REFERENCES T_USER_USR(USR_ID) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
CREATE TABLE TJ_CAT_RCP (
  ...
  FOREIGN KEY (CAT_ID) REFERENCES T_CATEGORY_CAT(CAT_ID) ON DELETE CASCADE,
  FOREIGN KEY (RCP_ID) REFERENCES T_RECIPE_RCP(RCP_ID) ON DELETE CASCADE,
...
CREATE TABLE TJ_IGD_RCP_UNT (
  ...
  FOREIGN KEY (RCP_ID) REFERENCES T_RECIPE_RCP(RCP_ID) ON DELETE CASCADE,
  FOREIGN KEY (IGD_ID) REFERENCES T_INGREDIENT_IGD(IGD_ID) ON DELETE CASCADE,
  FOREIGN KEY (UNT_ID) REFERENCES T_UNIT_UNT(UNT_ID) ON DELETE CASCADE,
...
CREATE TABLE T_COMMENT_COM (
  ...
  RCP_ID bigint(20) unsigned NOT NULL,
  PRIMARY KEY (COM_ID),
  FOREIGN KEY (USR_ID) REFERENCES T_USER_USR(USR_ID) ON DELETE SET NULL,
  FOREIGN KEY (RCP_ID) REFERENCES T_RECIPE_RCP(RCP_ID) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;
